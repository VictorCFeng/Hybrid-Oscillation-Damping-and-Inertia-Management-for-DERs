%% Using the solution of the standard problem (Scenario 1)
clear;clc;
%%
...
t_gfl = 35;                            % the time to switch GFL parameters
%%
...
for k =1:9
    ...
    D_p_pu = d(k)*Sb/JDb;               % Active power damping before switching [pu]
...
for k =1:9
    Init(k).theta0 = theta0(k); % Initia angles
end
...
c_f_pu = 0.05;                                         % output filter capacitor [pu] 
...
for k=1:3
    ...
    m_gfl(k) = m(kk)*Sb;                                  % inertia [normial]
...
for k=1:3
    kk = gfl_idx(k);
    Init_GFL(k).theta0 = theta0(kk); %Initial angles
end
%%
sim("simulation_model.slx",[0 60]);