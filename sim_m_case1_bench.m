%% Using the solution of the only frequency stability case (Scenario 2)
clear;clc;
%%
mpc = case_data;                       % load network data 
load case_1_data.mat                   % load M and D result (for starting the system)
ms = solution.m; ds = solution.d;    
load case_1_bench_data.mat             % load M and D result
...
line = mpc.line;                       % Line parameter (nomial value)
...
for k =1:9
    ...
    D_p_pu = ds(k)*Sb/JDb;              % Active power damping before switching [pu]
    ...
    %
    if m(k)>0.001   % avoid too small inertia given by optimization numerical results
...
for k=1:3
    ...
    m_gfl(k) = m(kk)*Sb;                                  % inertia (for starting the system)
    d_gfl(k) = d(kk)*Sb;                                  % active power damping [normial]
    w_pll = 0.1;                                          % bandwidth of PLL (before swithcing)
...
%%
sim("simulation_model.slx",[0 t_gfl+2]);